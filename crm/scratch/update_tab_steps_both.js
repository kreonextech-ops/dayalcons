...
// 1. Projects TabSteps
let pSteps = `import React, { useState, useEffect } from "react";
...
`;
fs.writeFileSync('src/views/admin/projects/components/TabSteps.jsx', pSteps);

// 2. Services TabSteps
let sSteps = `import React, { useState, useEffect } from "react";
import Card from "components/card";
import { createClient } from "@supabase/supabase-js";
import { MdCheckCircle, MdSave, MdRadioButtonUnchecked, MdAdd, MdDelete } from "react-icons/md";
...
const DEFAULT_STEPS = [
  { id: 1, order: 1, title: "Initial Consultation & Requirements Gathering", completed: false },
  { id: 2, order: 2, title: "Site Visit & Measurement", completed: false },
  { id: 3, order: 3, title: "Drafting Initial Plans / Design", completed: false },
  { id: 4, order: 4, title: "Client Review & Feedback", completed: false },
  { id: 5, order: 5, title: "Revisions & Final Adjustments", completed: false },
  { id: 6, order: 6, title: "Final Approval & Handover", completed: false }
];
  ...
  return (
    <div className="animate-fade-in flex flex-col gap-6">
      <Card extra="p-6 border border-[#E2E8F0] bg-white">
         <div className="flex justify-between items-center mb-6">
            <div>
               <h2 className="text-[20px] font-bold text-[#0F172A]">Service Workflow</h2>
               <p className="text-[14px] text-[#64748B]">Track progress through predefined service milestones.</p>
            </div>
            <div className="text-right">
               <div className="text-[24px] font-bold text-[#2563EB]">{progress}%</div>
               <div className="text-[12px] text-[#64748B] uppercase font-bold tracking-wider">Completed</div>
...
export default TabSteps;
`;
fs.writeFileSync('src/views/admin/services/components/TabSteps.jsx', sSteps);
