...
export default function AuditLogs() {
  ...
  const toggleExpand = (emp) => {
     setExpandedEmps(prev => prev.includes(emp) ? prev.filter(e => e !== emp) : [...prev, emp]);
  };
  ...
  filteredLogs.forEach(log => {
     const emp = log.employee_name || 'System / Unknown User';
    ...
    <div className="mt-5 w-full pb-12">
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
           ...
           />
           <button onClick={fetchLogs} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 shadow-sm transition dark:bg-navy-700 dark:border-white/10 dark:text-white dark:hover:bg-navy-600">
             <MdRefresh size={20} />
           </button>
        </div>
      </div>
      ...
      {loading ? (
         ...
         <div className="py-20 flex flex-col items-center justify-center text-center">
            <div className="bg-gray-100 text-gray-400 p-6 rounded-full mb-4 dark:bg-navy-800">
               <MdMonitor size={48} />
            </div>
            <h3 className="text-xl font-bold text-navy-700 dark:text-white">No logs found</h3>
            <p className="text-gray-500 mt-2 max-w-md">Try adjusting your search terms or verify that the system has correctly recorded employee actions.</p>
         </div>
      ) : (
         <div className="space-y-6">
            {employeeNames.map(emp => {
               const empLogs = grouped[emp];
               const isExpanded = expandedEmps.includes(emp);
               const displayLogs = isExpanded ? empLogs : empLogs.slice(0, 5);
               
               return (
                  <Card key={emp} extra="w-full p-0 overflow-hidden border border-gray-200 shadow-sm hover:shadow-md transition dark:border-white/10">
                     <div 
                     ...
                     >
                        <div className="flex items-center gap-4">
                           ...
                           <div>
                              <h3 className="text-lg font-bold text-navy-700 dark:text-white">{emp}</h3>
                              <p className="text-sm font-medium text-gray-500 flex items-center gap-2">
                                 ...
                                 <span>Last active: <span className="text-navy-700 dark:text-gray-300">{timeAgo(empLogs[0].created_at)}</span></span>
                              </p>
                           </div>
                        </div>
                     ...
                     {!isExpanded && empLogs.length > 5 && (
                        <div className="bg-gray-50/50 border-t border-gray-100 p-2.5 text-center dark:bg-navy-800 dark:border-white/10">
                           <p className="text-[12px] font-bold text-gray-400">
                              + {empLogs.length - 5} more activities. Click "View All" to expand.
                           </p>
                        </div>
                     )}
                  </Card>
