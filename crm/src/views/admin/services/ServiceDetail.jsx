...
import { 
  MdArrowBack, MdPerson, MdDesignServices, MdPhone, MdMessage, MdEmail, MdEvent, MdClose
} from "react-icons/md";
...
const ServiceDetail = ({ serviceCase, onBack, onUpdate }) => {
  ...
  const handleToggleAssignEmployee = async (employeeId) => {
    ...
    // Optimistic update
    const updatedCase = { ...serviceCase, assigned_to: newAssignedString || null };
    if (onUpdate) onUpdate(updatedCase);

    if (serviceCase.id) {
       await supabase.from('services').update({ assigned_to: newAssignedString || null }).eq('id', serviceCase.id);
    }
  };
  ...
  const handleQuickAction = (label) => {
    if (label === 'Schedule') {
       ...
       if (label === 'Call' && serviceCase.client?.phone) {
         window.open(`tel:${serviceCase.client.phone}`, '_self');
       } else if (label === 'WhatsApp' && serviceCase.client?.phone) {
         window.open(`https://wa.me/${serviceCase.client.phone.replace(/\D/g, '')}`, '_blank');
       } else if (label === 'Email' && serviceCase.client?.email) {
         window.open(`mailto:${serviceCase.client.email}`, '_self');
       }
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <div className="flex items-center gap-3">
               <h1 className="text-[28px] font-bold text-[#0F172A] dark:text-white">{serviceCase.title}</h1>
               <span className="bg-gray-100 dark:bg-navy-700 text-[#475569] dark:text-gray-200 px-3 py-1 rounded-md text-[12px] font-bold">SRV-{serviceCase.id?.substring(0,5).toUpperCase()}</span>
            </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                 ...
                 <Card extra="p-6 border border-[#E2E8F0] dark:border-navy-700 shadow-sm">
                   ...
                   <div className="space-y-4">
                     ...
                     <div className="flex flex-col"><span className="text-[12px] font-medium text-[#64748B] dark:text-gray-400">Contact Email</span><span className="text-[14px] font-semibold text-[#0F172A] dark:text-white">{serviceCase.client?.email || "—"}</span></div>
                     <div className="flex flex-col"><span className="text-[12px] font-medium text-[#64748B] dark:text-gray-400">Contact Phone</span><span className="text-[14px] font-semibold text-[#0F172A] dark:text-white">{serviceCase.client?.phone || "—"}</span></div>
                     <div className="flex flex-col"><span className="text-[12px] font-medium text-[#64748B] dark:text-gray-400">Project Creation Date</span><span className="text-[14px] font-semibold text-[#0F172A] dark:text-white">{new Date(serviceCase.created_at).toLocaleDateString('en-GB')}</span></div>
                   </div>
                 </Card>
                 ...
                 <Card extra="p-6 border border-[#E2E8F0] dark:border-navy-700 shadow-sm">
                   ...
                   <div className="space-y-4">
                     ...
                     <div className="flex flex-col"><span className="text-[12px] font-medium text-[#64748B] dark:text-gray-400">Primary Requirement</span><span className="text-[14px] font-semibold text-[#0F172A] dark:text-white">{serviceCase.title}</span></div>
                 ...
                 {isAdmin && (
                   <Card extra="p-6 border border-[#E2E8F0] dark:border-navy-700 shadow-sm">
                     ...
                     <div className="space-y-4">
                       <div className="flex justify-between items-center"><span className="text-[12px] font-medium text-[#64748B] dark:text-gray-400">Service Value</span><span className="text-[14px] font-bold text-[#0F172A] dark:text-white">₹ {totalAmount.toLocaleString()}</span></div>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white dark:bg-navy-800 p-6 shadow-2xl animate-fade-in">
            ...
            <form onSubmit={async (e) => {
              ...
              const { error } = await supabase.from('tasks').insert([{
                ...
                priority: 'High',
                status: 'To Do',
                assignee_id: serviceCase.assigned_to?.split(',')[0] || null, // assign to first employee
