...
import { 
  ...
  MdBusinessCenter, MdCurrencyRupee, MdAdd, MdCloudDownload, MdCloudUpload,
  MdMoreVert, MdFolder, MdChevronRight, MdChevronLeft, MdClose, MdCheckCircle, MdDelete
} from "react-icons/md";
...
const Services = () => {
     ...
     const fetchServices = async () => {
        ...
        if (!error && data) {
           ...
           const merged = data.map(srv => {
               const clientMatch = clientsData?.find(c => c.id === srv.client_id);
               return { ...srv, client: clientMatch || { name: 'Unknown Client' } };
           });
           setServices(merged);
        }
        setLoading(false);
  ...
  const handleDelete = async (e, id) => {
     e.stopPropagation();
     if (window.confirm("Are you sure you want to delete this service case?")) {
  ...
  const handleCreate = async (e) => {
     ...
     const title = newCase.selectedServices.length > 0 ? newCase.selectedServices[0] + (newCase.selectedServices.length > 1 ? ' & Others' : '') : "Unnamed Service";
     
     const metadata = {
        requirements: newCase.selectedServices,
        financials: {
           total: newCase.serviceCharge || 0,
     ...
     if (srvError) {
        alert("Failed to create service case: " + srvError.message);
        return;
     }

     setNewCase({
       clientType: "new", clientName: "", phone: "", whatsapp: "", email: "", address: "", clientId: "",
       selectedServices: [], serviceCharge: "", advanceAmount: "", targetDate: ""
     });
     setModalStep(1);
  ...
  const toggleService = (id) => {
      setNewCase(prev => {
          const arr = prev.selectedServices || [];
          if (arr.includes(id)) return { ...prev, selectedServices: arr.filter(x => x !== id) };
          return { ...prev, selectedServices: [...arr, id] };
      });
  };

  if (selectedCase) {
    return <ServiceDetail serviceCase={selectedCase} onBack={() => setSelectedCase(null)} onUpdate={(updated) => { setSelectedCase(updated); setRefreshTrigger(p => p + 1); }} />;
  }
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4 mt-8 md:mt-2">
          <div>
            ...
            <h1 className="text-[32px] font-bold text-[#0F172A] dark:text-white leading-tight">Consultancy Services</h1>
               ...
               <tbody>
                  {(() => {
                     ...
                     let mapped = filtered.map(srv => {
                        let prog = 0;
                        try {
                           const meta = ( () => { try { return JSON.parse(srv.description || "{}"); } catch(e) { return { old_description: srv.description }; } } )();
                     ...
                     <div className="animate-fade-in">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                          {DESIGN_SERVICES.map(srv => {
                             const checked = (newCase.selectedServices || []).includes(srv.id);
                             return (
                               <div 
                                 key={srv.id} 
