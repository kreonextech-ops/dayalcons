...
const SiteVisits = () => {
  ...
  const handleCreate = async (e) => {
    ...
    if (!error) {
      setShowModal(false);
      setNewVisit({ client_name: '', mobile_no: '', location: '', visiting_date: '', requirement: '', amount: '', status: 'Ongoing' });
      fetchVisits();
    } else {
  ...
  return (
    <div className="mt-3 grid h-full grid-cols-1 gap-5">
      <Card extra="w-full pb-10 p-6 h-full">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-[#0F172A] dark:text-white">Site Visits Register</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan="8" className="py-4 text-center">Loading site visits...</td></tr>
              ) : visits.length === 0 ? (
                <tr><td colSpan="8" className="py-4 text-center text-gray-500">No site visits found.</td></tr>
              ) : (
                visits.map(v => (
                  <tr key={v.id} className="border-b border-gray-50 dark:border-navy-700 hover:bg-gray-50 dark:hover:bg-navy-800 transition">
                    <td className="py-4 pr-4 font-bold text-[#0F172A] dark:text-white">{v.client_name || '-'}</td>
                    <td className="py-4 pr-4 text-sm text-gray-600 dark:text-gray-200">{v.mobile_no || '-'}</td>
                    <td className="py-4 pr-4 text-sm text-gray-600 dark:text-gray-200">{v.location || '-'}</td>
          ...
          <div className="bg-white dark:bg-navy-800 rounded-[20px] p-6 w-full max-w-2xl shadow-2xl">
             ...
             <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={newVisit.client_name} onChange={e=>setNewVisit({...newVisit, client_name: e.target.value})} className="w-full h-11 px-3 border rounded-[10px] dark:bg-navy-900 dark:border-navy-700 dark:text-white" required />
                </div>
                <div>
