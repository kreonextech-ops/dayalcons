...
if 'Lead Source' not in content.split('Add New Client')[1]:
    content = content.replace('                  <div>\n                    <label className="block text-[12px] font-bold text-[#475569] dark:text-gray-200 dark:text-white mb-1.5 uppercase tracking-wide">GST / PAN (Tax ID)</label>', source_block + '\n                  <div>\n                    <label className="block text-[12px] font-bold text-[#475569] dark:text-gray-200 dark:text-white mb-1.5 uppercase tracking-wide">GST / PAN (Tax ID)</label>')

with open(filepath, 'w', encoding='utf-8') as f: f.write(content)

print("Clients index.jsx patched successfully")
